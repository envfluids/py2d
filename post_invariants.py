...
for tcount in range(10):#omega_M_2D.shape[2]):
...
#     sr_points, n_neighbors=100, n_components=3
# )

# fig, axs = plt.subplots(figsize=(8, 8), nrows=2)
# axs[0].scatter(sr_lle[:, 0], sr_lle[:, 1], c=sr_color)
# axs[0].set_title("LLE Embedding")

# axs[1].scatter(sr_lle[:, 1], sr_lle[:, 2], c=sr_color)
...
for PERPLEXITY in [100]:#[1,5,10, 100,500,1e9]:#, 5000, 10000]:
    ...
    for icount in range(0,10):
        ...
        # plt.savefig('C'+str(CASENO)+'_Nx'+str(NX)+'_'+METHOD+'_p'+str(PERPLEXITY)+'l'+str(icount)+'.png',dpi=DPI)
