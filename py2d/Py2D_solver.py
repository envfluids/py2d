...
def Py2D_solver(
    ...
    """
    Solves 2D Navier-Stokes in an iterative loop based on input parameters.
    ...
    Re : float
        Reynolds number
    fkx : float
        Forcing wavenumber in x
    fky : float
        Forcing wavenumber in y
    alpha : float
        Rayleigh drag coefficient
    beta : float
        Coriolis parameter
    NX : int
        Number of grid points in x and y '32', '64', '128', '256', '512'
    SGSModel_string : str
        SGS model to use 'NoSGS', 'SMAG', 'DSMAG', 'LEITH', 'DLEITH', 'PiOmegaGM2',
        'PiOmegaGM4', 'PiOmegaGM6'
    eddyViscosityCoeff : float
        Coefficient for eddy viscosity models: SMAG and LEITH
    dt : float
        Time step
    saveData : bool
        Save data?
    ...
    readTrue : bool
        ????
    ICnum : int
        Initial condition number: Choose between 1 to 20
    resumeSim : bool
        Sart new simulation (False) or resume simulation (True)
    ...
    # SGS Model - PiOmega numerical scheme (Crank nicholson scheme used for time
    #                                       integration and eddy viscosity)
    ...
    SAVE_DIR, SAVE_DIR_DATA, SAVE_DIR_IC = gen_path(
        NX, dt, ICnum, Re, fkx, fky, alpha, beta, SGSModel_string
    )
    ...
    PiOmega_eddyViscosity_model = SGSModel(
        Kx, Ky, Ksq, Delta, method=SGSModel_string, C_MODEL=eddyViscosityCoeff, dealias=dealias
    )
    # PiOmega_eddyViscosity_model.set_method(SGSModel_string) # Set SGS model to
    # calculate PiOmega and Eddy Viscosity
    ...
    if readTrue:
        ...
        if last_file_number_IC is not None:
            ...
            msg = "No .mat initialization files found to resume the simulation"
            raise ValueError(msg)

        # Load initial condition to resume simulation
        # Resume from the second last saved file -
        # the last saved file is often corrupted since the jobs stop (reach wall
        # clocktime limit) while the file is being saved.
        ...
        if last_file_number_data is None:
            ...
            msg = (
                "Data already exists in the results folder for this case, either "
                "resume the simulation (resumeSim = True) or delete data to start a "
                "new simulation"
            )
            raise ValueError(msg)
    ...
    for it in range(maxit):
        ...
        #     output_normalized = PiOmega_eddyViscosity_model.calculate(cnn_model,
        #                             input_data=input_data, Kx=Kx, Ky=Ky, Ksq=Ksq)
        #     # # Diagnosis
        ...
        #     # output_denomralized = denormalize_data(output_normalized,
        #     #                                        mean= output_mean,
        #     #                                        std= output_std)
        #     # output_denomralized = np.zeros((3, output_normalized.shape[1],
        #     #                                 output_normalized.shape[2]))
        #     # for i in range(3):
        #     #     updated_values = denormalize_data(output_normalized[i],
        #     #                                       mean= output_mean[i],
        #     #                                       std= output_std[i])
        #     #     output_denomralized = output_denomralized.at[i, :, :].set(
        #     #                               updated_values
        #     #                           )
        #     PiOmega1_hat = postproccess_data_cnn_jit(output_normalized[0],
        #                                              output_normalized[1],
        #                                              output_normalized[2],
        #                                              Kx, Ky, Ksq)
        ...
        # RHS = Omega1_hat + dt*(-1.5*convec1_hat + 0.5*convec0_hat) +
        #         dt*0.5*(nu+ve)*diffu_hat + dt*(Fk_hat-PiOmega_hat)
        ...
        if saveData and np.mod(it + 1, (NSAVE)) == 0:
            ...
            print(
                (
                    "Time = {:.6f} -- Eddy Turnover Time = {:.6f} -- C = {:.4f} "
                    "-- Eddy viscosity = {:.6f} ** Saved {}"
                ).format(
                    time,
                    eddyTurnoverTime,
                    eddyViscosityCoeff,
...
if __name__ == "__main__":
    ...
    for SGSModel_string in SGSModel_list:
        ...
        Py2D_solver(
            ...
            SGSModel_string=SGSModel_string,  # SGS model to use 'NoSGS', 'SMAG',
            # 'DSMAG', 'LEITH', 'DLEITH',
            # 'PiOmegaGM2', 'PiOmegaGM4', 'PiOmegaGM6'
            eddyViscosityCoeff=0.17,  # Coefficient for eddy viscosity models: SMAG and
            # LEITH
            dt=5e-4,  # Time step
