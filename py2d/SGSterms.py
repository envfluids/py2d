...
def Tau(Omega_DNS, filterType='gaussian', coarseGrainType='spectral', Delta=None, N_LES=None):
    ...
    U_f = filter2D_2DFHIT(U_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    V_f = filter2D_2DFHIT(V_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)

    # Filter the products of the fields
    # This involves multiplying the fields with each other and then applying the filter
    UU_f = filter2D_2DFHIT(U_DNS*U_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    VV_f = filter2D_2DFHIT(V_DNS*V_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    UV_f = filter2D_2DFHIT(U_DNS*V_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
...
def Sigma(Omega_DNS, filterType='gaussian', coarseGrainType='spectral', Delta=None, N_LES=None):
    ...
    Omega_f = filter2D_2DFHIT(Omega_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)

    # Filter the products of the velocity fields and the vorticity field
    UOmega_f = filter2D_2DFHIT(U_DNS*Omega_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    VOmega_f = filter2D_2DFHIT(V_DNS*Omega_DNS, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
...
def PiUV(Omega_DNS, filterType='gaussian', coarseGrainType='spectral', Delta=None, N_LES=None):
    ...
    UUx_f = filter2D_2DFHIT(UUx, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    UVy_f = filter2D_2DFHIT(UVy, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    UVx_f = filter2D_2DFHIT(UVx, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    VVy_f = filter2D_2DFHIT(VVy, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
...
def PiOmega(Omega_DNS, filterType='gaussian', coarseGrainType='spectral', Delta=None, N_LES=None):
    """
    Function to calculate PiOmega based on the given velocity fields (U_DNS, V_DNS) and slnWorDNS.
    This involves calculating the divergence of the Sigma vector in conservative form.
    Non-conservative form is also available.

    Parameters:
    ------
    U_DNS, V_DNS (numpy.ndarray): 2D velocity fields.
    slnWorDNS (numpy.ndarray): Solution Wor DNS array.
    filterType (str): Type of filter to use ('gaussian', etc.).
    Delta (float): Filter width - positive real number.
    N_LES (numpy.ndarray): Size of coarse (LES) grid - 2x1 array.
    ...
    UOmega_x_f = filter2D_2DFHIT(UOmega_x, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
    VOmega_y_f = filter2D_2DFHIT(VOmega_y, filterType=filterType, coarseGrainType=coarseGrainType, Delta=Delta, N_LES=N_LES)
