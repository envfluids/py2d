...
# from mypathdictionary3_compare import *; pc=True
# from mypathdictionary3_compare import *; pc=True
from mypathdictionary3_compare_CC import *; pc=True
...
for file in natsorted(os.listdir(path_RL), alg=ns.PATH | ns.IGNORECASE):
    ...
    if file.endswith('.mat') and filecount%1==0:
        ...
        #
        ve_RL = mat_contents_RL['veRL']
        ve_smag = mat_contents_RL['vesmag']
        #
        cs_RL = mat_contents_RL['cdynamic']*(2*np.pi/NX)**2
        cs_smag = mat_contents_RL['csmag']
        #
        ...
        if (cs_RL.shape)[0]==NX:
            ...
            S_M = np.append(S_M, S)
            #
            ve_RL_M = np.append(ve_RL_M, ve_RL)
            ve_smag_M = np.append(ve_smag_M, ve_smag)
            #
            cs_RL_M = np.append(cs_RL_M, cs_RL)
            cs_local_M = np.append(cs_local_M, cs_smag)
            # stop
...
for icount in range(len(camview.T)):
    ...
    # ax.scatter(S_M, abs_grad_omega_M, cs_local_M, s= 2, c= cs_local_M, vmin=VMIN, vmax=VMAX, alpha=ALPHA)
    ax.scatter(S_M, abs_grad_omega_M, ve_smag_M, s= 2, c= ve_smag_M, vmin=-0.001, vmax=0.001, alpha=0.15)
    ...
    # ax.scatter(S_M, abs_grad_omega_M, cs_RL_M, s= 2, c= cs_RL_M, vmin=VMIN, vmax=VMAX, alpha=ALPHA)
    ax.scatter(S_M, abs_grad_omega_M, ve_RL_M, s= 2, c= ve_RL_M, vmin=-0.001, vmax=0.001, alpha=0.15)
